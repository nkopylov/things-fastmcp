...
def retry_operation(func, max_retries=3, delay=1, operation_name=None, params=None):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.warning(
                    f"Attempt {attempt + 1} failed. Retrying in {wait_time:.2f} seconds: {str(e)}"
                )
    ...
    if operation_name and params and last_exception:
        dead_letter_queue.add_failed_operation(
            operation_name, params, str(last_exception), attempts=max_retries
        )
...
async def handle_tool_call(
    ...
    try:
        ...
        # Project operations
            ...
            include_items = (
                arguments.get("include_items", False) if arguments else False
            )
        ...
        # Tag operations
            ...
            if not todos:
                return [
                    types.TextContent(
                        type="text", text=f"No items found with tag '{tag}'"
                    )
                ]
        ...
        # Search operations
            ...
            if not todos:
                return [
                    types.TextContent(
                        type="text", text=f"No todos found matching '{query}'"
                    )
                ]
        ...
        # Recent items
            ...
            if not todos:
                return [
                    types.TextContent(
                        type="text", text=f"No items found in the last {period}"
                    )
                ]
        ...
        # Things direct AppleScript operations
            ...
            # list_title no longer used
            # arguments.get("list_title")
            ...
            simple_params = {"title": title, "notes": notes, "when": when, "tags": tags}
            ...
            if not task_id:
                ...
                try:
                    ...
                    return [
                        types.TextContent(
                            type="text", text=f"⚠️ Error in retry: {str(e)}"
                        )
                    ]
            ...
            if not task_id:
                ...
                return [
                    types.TextContent(
                        type="text", text=f"⚠️ Error: Failed to create todo: {title}"
                    )
                ]

            return [
                types.TextContent(
                    type="text", text=f"✅ Created new todo: {title} (ID: {task_id})"
                )
            ]
            ...
            try:
                ...
                if success:
                    return [
                        types.TextContent(
                            type="text",
                            text=f"✅ Created new project: {arguments['title']}",
                        )
                    ]
            except Exception as e:
                logger.warning(
                    f"X-Callback add-project failed: {str(e)}, falling back to URL scheme"
                )
            ...
            return [
                types.TextContent(
                    type="text", text=f"Created new project: {arguments['title']}"
                )
            ]
            ...
            tag_only_update = tag_update_needed and all(
                v is None for k, v in params.items() if k not in ["id", "tags"]
            )

            # Log the tag update details
            if tag_update_needed:
                logger.info(
                    f"Tag update needed: {arguments['tags']} for todo ID: {arguments['id']}"
                )
            ...
            if tag_only_update:
                logger.info(
                    f"Using hybrid tag management approach for todo: {arguments['id']}"
                )
                ...
                try:
                    ...
                    if not isinstance(tags, list) or not tags:
                        logger.warning(
                            f"Invalid tags format or empty tags list: {tags}"
                        )
                        return False
                    ...
                    for tag in tags:
                        ...
                        add_tag_url = url_scheme.update_todo(
                            id=todo_id, add_tags=tag_str
                        )
                    ...
                    if all_tags_added:
                        logger.info(
                            f"All tags successfully added to todo ID: {todo_id}"
                        )
                        success = True
                    else:
                        logger.warning(
                            f"Some tags failed to be added to todo ID: {todo_id}"
                        )
                ...
                logger.info(
                    f"Using direct AppleScript implementation to update todo: {arguments['id']}"
                )
                success = retry_operation(
                    lambda: applescript_bridge.update_todo_direct(**params),
            ...
            if not success:
                logger.error(
                    f"All update methods failed for todo with ID: {arguments['id']}"
                )
            ...
            return [
                types.TextContent(
                    type="text",
                    text=f"✅ Successfully updated todo with ID: {arguments['id']}",
                )
            ]
            ...
            try:
                ...
                if success:
                    return [
                        types.TextContent(
                            type="text",
                            text=f"✅ Successfully updated project with ID: {arguments['id']}",
                        )
                    ]
            except Exception as e:
                logger.warning(
                    f"X-Callback update-project failed: {str(e)}, falling back to URL scheme"
                )
            ...
            if not success:
                raise RuntimeError(
                    f"Failed to update project with ID: {arguments['id']}"
                )
            return [
                types.TextContent(
                    type="text",
            ...
            return [
                types.TextContent(
                    type="text",
                    text=f"Successfully opened item with ID: {arguments['id']}",
                )
            ]
        ...
        if arguments:
            dead_letter_queue.add_failed_operation(name, arguments, str(e))
